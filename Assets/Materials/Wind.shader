...
Properties {
    ...
    _ShakeTime ("Shake Time", Range (0, 100.0)) = 1.0
...
void vert (inout appdata_full v) {
    ...
    v.vertex.z -= mul ((float3x3)unity_WorldToObject, waveMove).xz;

    float3 worldPos = mul(unity_ObjectToWorld, float4(v.vertex.xyz, 1)).xyz;
    
    //worldPos += playerToVertexDir * (1 - saturate(playerToVertexDist / _BendRange)) * _BendStrength;
    float3 ran = rand(float3(1, 1, 1));

    v.vertex.xyz += v.normal * sin(_Time.y * _ShakeTime) * worldPos * _ShakeDisplacement * ran;
