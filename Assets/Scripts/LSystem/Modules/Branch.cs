...
namespace CKZH.ProcedualTree
{
    public class Branch : Module
    {
        ...
        public override void Generate(Seed seed)
        {
			...
			float newLength = length * seed.currentTransform.currentScale;

			Vector3 direction = startRot * transform.up;
            Vector3 endPos = startPos + (direction * newLength);
            Quaternion endRot = Quaternion.FromToRotation(startPos, endPos);

			var endWidth = seed.currentTransform.currentBranchWidth * seed.currentTransform.currentScale * scaleReduction;
			DrawCylinder(newLength * 1.1f, seed.currentTransform.currentBranchWidth * seed.currentTransform.currentScale, endWidth);
			...
			seed.currentTransform.position = endPos;
        ...
        private void DrawCylinder(float height, float bottomRadius, float topRadius)
        {
